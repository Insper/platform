...
sequenceDiagram
  ...
  Note over N1,N3: Term 1 — leader is silent, election timeout fires on Node 1
  ...
  Note over N1: majority (2/3) — becomes Leader for term 2
  N1->>N2: AppendEntries(term=2, heartbeat)
  N1->>N3: AppendEntries(term=2, heartbeat)
```

A candidate wins the election only if it receives votes from a majority of nodes **and** its log is at least as up-to-date as the voters' logs (the *election restriction*). This ensures the new leader already holds all committed entries.
